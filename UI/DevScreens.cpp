...
void JitCompareScreen::CreateViews() {
	...
	lin->Add(new Choice("Rand"))->OnClick.Handle(this, &JitCompareScreen::OnRandomBlock);
	lin->Add(new Choice("VFPU"))->OnClick.Handle(this, &JitCompareScreen::OnRandomVFPUBlock);
...
void JitCompareScreen::UpdateDisasm() {
	...
	if (currentBlock_ < 0 || currentBlock_ >= blockCache->GetNumBlocks()) {
		...
		irDisasm_->Add(new TextView("No block"));
		blockStats_->SetText("");
		return;
	}
